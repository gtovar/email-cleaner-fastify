...
- The fixture inbox content now includes extractor-ready receipt bodies so the local browser flow can validate one successful manual WhatsApp send path without changing the HTTP contract.
...
- Updated the fixture inbox content to include extractor-ready receipt bodies for the local HU06 browser validation path; `tests/emailsFixtureRoutes.integration.test.js` passed locally after the fixture change (commit: pending).
